...
function HeroSection({ chef }: { chef: any }) {
  ...
  return (
    <View className="items-center mb-6">
      ...
      <Text className="text-2xl font-bold text-gray-900 mt-3">{name}</Text>
      {chef.tagline && (
        <Text className="text-sm text-gray-500 mt-1 text-center px-4">{chef.tagline}</Text>
      )}
      ...
      {chef.specialties && chef.specialties.length > 0 && (
        <View className="flex-row flex-wrap justify-center gap-2 mt-3 px-4">
          {chef.specialties.map((s: string) => (
            <View key={s} className="bg-gray-100 px-2.5 py-1 rounded-full">
              <Text className="text-xs font-medium text-gray-700">{s}</Text>
            </View>
          ))}
        </View>
      )}
    ...
    <View className="border border-gray-100 rounded-2xl p-4 mb-4">
      <View className="flex-row justify-between items-center mb-2">
        ...
        <View className="bg-amber-100 px-2 py-0.5 rounded-full">
          <Text className="text-xs font-semibold text-amber-700">Featured</Text>
        </View>
      </View>
      <View className="flex-row items-center gap-3 mb-3">
        ...
        <Text className="text-sm font-bold text-gray-900">${menu.base_price}</Text>
      </View>
      {menu.dishes && menu.dishes.length > 0 && (
        <View className="mb-3">
          {menu.dishes.slice(0, 3).map((dish) => (
            <Text key={dish.id} className="text-sm text-gray-600 mb-0.5">
              {dish.name}
            </Text>
          ))}
      ...
      <Pressable className="bg-gray-900 rounded-xl py-3 items-center" onPress={onOrder}>
...
function MenuListSection({ menus }: { menus: WeeklyMenuSummary[] }) {
  ...
  return (
    <View className="mb-4">
      <Text className="text-lg font-bold text-gray-900 mb-3">Weekly Menus</Text>
      {menus.map((menu) => (
        <View key={menu.id} className="border border-gray-100 rounded-xl p-3 mb-2">
          <Text className="text-sm font-bold text-gray-900">{menu.title}</Text>
          <View className="flex-row items-center gap-3 mt-1">
            <Text className="text-xs text-gray-500">{menu.duration_days} days</Text>
            <Text className="text-xs text-gray-500">{getMenuScopeLabel(menu.menu_scope)}</Text>
            <Text className="text-xs font-semibold text-gray-700">${menu.base_price}</Text>
          </View>
        </View>
      ))}
    </View>
  ...
  if (!data?.chef) {
    return (
      <View className="flex-1 items-center justify-center bg-white">
        ...
        <Text className="text-lg font-semibold text-gray-800 mt-3">Chef not found</Text>
      </View>
    );
  }
    ...
    <ScrollView className="flex-1 bg-white" contentContainerClassName="px-4 py-4 pb-20">
      ...
      {chef.bio && (
        <View className="mb-6">
          ...
          <Text className="text-sm text-gray-600 leading-5">{chef.bio}</Text>
        </View>
      )}

      {!weekly_menus?.length && !featured_menu && (
        <View className="border border-dashed border-gray-300 rounded-2xl p-6 items-center mb-4">
      ...
      <Pressable
        className="bg-gray-900 rounded-xl py-3.5 items-center mt-4"
        onPress={() =>
          navigation.navigate('OrderBuilder', { slug: chef.slug ?? String(chef.id) })
        }
      >
